...
m = float(2.7*math.pow(10,-3)) # when rebounded the value was 3
# m = float(1.0*math.pow(10, -3))
Cd = float(0.62) # when rebounded the value was 0.54
# Cm = float(0.069)
# Cd=0
Cm = float(0.0)
...
def dist(t, s, v, w):
    ...
    W0 = w
    V0 = v
    var = RK45(vel, 0, S0, t, max_step=(max_step))
    return var
...
def mains(mode, s_0, v_0, w_0, step, base):
    global t0, max_step, g
    ...
    while var.status != "finished":
        ...
        if mode == 'table':
            ...
            if var.y[1] >= 1.5 and not base:
                break
            elif var.y[2] <= 0 and base:
                break
        else:
